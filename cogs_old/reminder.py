...
class Erinnerungen(commands.Cog):
        ...
        if str(ctx.author.id) in list(reminder.keys()) and len(reminder[str(ctx.author.id)]) > 0:
            ...
            for single_reminder in reminder[str(ctx.author.id)]:
                ...
                if single_reminder[0] == "{":
                    ...
                    guild = self.bot.get_guild(public_config.SERVER_ID)
    ...
    async def checkReminder(self):
        ...
        for recipient_id in recipients:
            for reminder in r[recipient_id]:
                channel = self.bot.get_channel(public_config.BOT_CHANNEL_ID)
                # old reminder format
                try:
                    ...
                    if time <= now:
                        author = self.bot.get_guild(
                            public_config.SERVER_ID).get_member(int(reminder[2]))
                        recipient = self.bot.get_guild(
                            public_config.SERVER_ID).get_member(int(recipient_id))
                        if recipient is None:
                            recipient = self.bot.get_guild(
                                public_config.SERVER_ID).get_role(int(recipient_id))
                        if recipient is None:
                            return
    ...
    async def beforeReminderCheck(self):
        ...
        channel = self.bot.get_channel(public_config.LOG_CHANNEL_ID)
    ...
    async def ReminderCheckError(self, error):
        ...
        await on_command_error(self.bot.get_channel(public_config.LOG_CHANNEL_ID), error)
...
def update_reminder(reminder):
    with open(public_config.path + '/json/reminder.json', 'w') as myfile:
        json.dump(reminder, myfile)


def get_reminder():
    with open(public_config.path + '/json/reminder.json', 'r') as myfile:
...
async def setup(bot):
    if(public_config.GUILDS):
        await bot.add_cog(Erinnerungen(bot), guilds=public_config.GUILDS)
    else:
